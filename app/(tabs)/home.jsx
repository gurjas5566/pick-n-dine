...
import banner from "../../assets/images/homeBanner2.png";
...
const home = () => {
  ...
  const renderItem = ({ item }) => (
    ...
    >
      ...
      />
      <Text className="text-[#FFD580] text-lg font-bold mb-2">{item.name}</Text>
  ...
  return (
    ...
    >
      <View className="flex items-center">
        <View className="bg-[#5f5f5f] w-11/12 rounded-lg shadow-lg justify-between items-center flex flex-row p-2">
          <View className="flex flex-row">
            <Text className="text-base h-10 p-[8] align-middle text-white font-bold">
              Welcome to PicknDine
            </Text>
          </View>
        </View>
      </View>
      <ScrollView stickyHeaderIndices={[0]}>
        ...
        >
          <BlurView
            intensity={Platform.OS == "android" ? 100 : 10}
            tint="dark"
            className="w-full p-4 shadow-lg"
          >
            <Text className="text-center text-2xl font-bold text-white">
              Pick and dine, with food so fine.
            </Text>
          </BlurView>
        </ImageBackground>
        ...
        {restaurants.length > 0 ? (
          ...
          <ActivityIndicator animating color={"#6B8E23"} />
        )}
        <View className="p-4 bg-[#2b2b2b] flex-row items-center">
          <Text className="text-3xl text-[#6B8E23] mr-2 font-semibold">
            Our Restaurants
          </Text>
        </View>
        {restaurants.length > 0 ? (
